...
func TestReadAllIncludesNetworkAndStorageRates(t *testing.T) {
	...
	if got := leaves["/network/eth0/throughput/0"].Value; got != "8388608 B/s" {
		t.Fatalf("network rx throughput = %q, want %q", got, "8388608 B/s")
	}
	if got := leaves["/network/eth0/throughput/1"].Value; got != "16777216 B/s" {
		t.Fatalf("network tx throughput = %q, want %q", got, "16777216 B/s")
	}
	if got := leaves["/network/eth0/data/0"].Value; got != "1090519040 B" {
		t.Fatalf("network rx total = %q, want %q", got, "1090519040 B")
	}
	if got := leaves["/network/eth0/data/1"].Value; got != "2181038080 B" {
		t.Fatalf("network tx total = %q, want %q", got, "2181038080 B")
	}
}
